  ...
  await ensureFile(REVIEWS_FILE, [
    {
      ...
      provider: "Google",
      rating: 5,
      text: "Очень сильный центр. Понравились преподаватели и атмосфера.",
      approved: true,
    ...
    },
    {
      ...
      provider: "Yandex",
      rating: 5,
      text: "Удобно, что есть языки и предметы в одном месте.",
      approved: true,
      createdAt: "02.01.2026, 11:00:00"
    }
  ]);
}
...
app.get("/api/applications", async (req, res) => {
  res.json(await readJson(APPLICATIONS_FILE, []));
});
...
app.delete("/api/applications/:id", async (req, res) => {
  const items = await readJson(APPLICATIONS_FILE, []);
  await writeJson(APPLICATIONS_FILE, items.filter((x) => x.id !== req.params.id));
  res.json({ ok: true });
});

app.get("/api/reviews", async (req, res) => {
  const onlyApproved = req.query.all !== "1";
  const items = await readJson(REVIEWS_FILE, []);
  res.json(onlyApproved ? items.filter((x) => x.approved) : items);
});

app.post("/api/reviews", async (req, res) => {
  const { author, provider, rating, text } = req.body || {};
  if (!author || !provider || !text) {
    return res.status(400).json({ ok: false, error: "Author, provider and text are required" });
  }
  if (!["Google", "Yandex"].includes(provider)) {
    return res.status(400).json({ ok: false, error: "Provider must be Google or Yandex" });
  }
  const items = await readJson(REVIEWS_FILE, []);
  const review = {
    id: Date.now().toString(),
  ...
  await writeJson(REVIEWS_FILE, items);
  res.json({ ok: true, review });
});

app.put("/api/reviews/:id/approve", async (req, res) => {
  const items = await readJson(REVIEWS_FILE, []);
  const next = items.map((x) => x.id === req.params.id ? { ...x, approved: true } : x);
  await writeJson(REVIEWS_FILE, next);
  res.json({ ok: true });
});

app.delete("/api/reviews/:id", async (req, res) => {
  const items = await readJson(REVIEWS_FILE, []);
  await writeJson(REVIEWS_FILE, items.filter((x) => x.id !== req.params.id));
  res.json({ ok: true });
});
